def LostContact(Pairs1, Pairs_tsl2, Reg1):
    """
    Detect lost contacts between two consecutive time steps.

    A contact is considered lost if both labels still exist at t+1
    but the pair itself no longer exists.

    :param Pairs1: Contact pairs at time t
    :type Pairs1: dict
    :param Pairs_tsl2: Contact pairs at time t+1 after tracking
    :type Pairs_tsl2: dict
    :param Reg1: Region properties at time t (labels and coordinates)
    :type Reg1: dict

    :return: Dictionary containing lost contact labels and coordinates
    :rtype: dict
        - clab : list of [lab1, lab2]
        - cx   : list of [x1, x2]
        - cy   : list of [y1, y2]
        - cz   : list of [z1, z2]
    """
...
def NewContact(Pairs1, Pairs_tsl2):
    """
    Detect new contacts appearing between two consecutive time steps.

    A contact is considered new if both labels existed at time t
    but the contact pair did not.

    :param Pairs1: Contact pairs at time t
    :type Pairs1: dict
    :param Pairs_tsl2: Contact pairs at time t+1 after tracking (extended)
    :type Pairs_tsl2: dict

    :return: Dictionary containing new contact labels and coordinates
    :rtype: dict
        - clab : list of [lab1, lab2]
        - cx   : list of [x1, x2]
        - cy   : list of [y1, y2]
...
def LostNewContact_Batch(pairsdirname, pairstrldirname, savedirnamelost, regdirname, savedirnamenew, imrange, verbose=True):   
    """
    Batch computation of lost and new contacts over a sequence of images.

    Results are written as TSV files for each image pair (t, t+1).

    :param pairsdirname: Directory info for original contact pairs
    :type pairsdirname: tuple
    :param pairstrldirname: Directory info for tracked contact pairs
    :type pairstrldirname: tuple
    :param savedirnamelost: Output directory for lost contacts
    :type savedirnamelost: tuple
    :param regdirname: Directory info for region properties
    :type regdirname: tuple
    :param savedirnamenew: Output directory for new contacts
    :type savedirnamenew: tuple
...
def Read_lostnew(readdirname, imrange, verbose=True):
    """
    Read lost or new contact TSV files into Python dictionaries.

    :param readdirname: Directory information for TSV files
    :type readdirname: tuple
    :param imrange: Image indices
    :type imrange: list or array
    :param verbose: Print file paths while reading
    :type verbose: bool

    :return: List of dictionaries containing contact labels and coordinates
    :rtype: list of dict
    """
...
def DetectT1_LostToNew(cont, lost, new, lcoord1,lcoord2, ncoord1,ncoord2):
    """
    Detect T1 events by linking lost contacts to newly formed contacts.

    For each lost contact, neighbors are searched to identify
    newly created contacts consistent with a T1 transition.

    :param cont: All contacts at time t
    :type cont: dict
    :param lost: Lost contact label pairs
    :type lost: list
    :param new: New contact label pairs
    :type new: list
    :param lcoord1: Coordinates of first label of lost contacts
    :type lcoord1: list
    :param lcoord2: Coordinates of second label of lost contacts
    :type lcoord2: list
    :param ncoord1: Coordinates of first label of new contacts
    :type ncoord1: list
    :param ncoord2: Coordinates of second label of new contacts
    :type ncoord2: list
...
def DetectT1_NewToLost(cont, lost, new, lcoord1,lcoord2, ncoord1,ncoord2):
    """
    Detect T1 events by linking new contacts to previously lost contacts.
...
def DetectT1_Batch(readdirnameall, readdirnamelost, readdirnamenew, namesave, dirsave, imrange, verbose=False, n0=3):
    """
    Batch detection of T1 events over a sequence of images.

    T1 events are detected both from lost-to-new and new-to-lost
    contact transitions and saved as pickle files.

    :param readdirnameall: Directory of all contacts
    :type readdirnameall: tuple
    :param readdirnamelost: Directory of lost contacts
    :type readdirnamelost: tuple
    :param readdirnamenew: Directory of new contacts
    :type readdirnamenew: tuple
    :param namesave: Base name for output files
    :type namesave: str
    :param dirsave: Output directory
    :type dirsave: str
    :param imrange: Image indices
    :type imrange: list or array
    :param verbose: Print progress information
...
def ReadT1(readdirname,imrange, verbose=False, n0=3):
    """
    Read T1 event pickle files for a sequence of images.

    :param readdirname: Directory containing T1 pickle files
    :type readdirname: tuple
    :param imrange: Image indices
    :type imrange: list or array
    :param verbose: Print file paths while reading
    ...
    :return: List containing New→Lost and Lost→New T1 events
    :rtype: list
    """
