...
def CutSerie(directory, name, imrange, cutzyx = [0,None], nameaxes=['z','y','x'], Ncolumns=1, title='',figblocksize=5,endread='.tiff',n0=3, cmap='gray', interpolation=None, crop=[0,-1]):
     """
    Display cut-planes from a series of 3D images in a grid of subplots.

    For each image in the series, a 2D slice is extracted along the selected
    axis (z, y, or x) and displayed in a matplotlib figure.

    :param directory: Directory containing the image series
    :type directory: str
    :param name: Base name of the images
    :type name: str
    :param imrange: List of image indices to display
    :type imrange: list or numpy.ndarray
    :param cutzyx: Cut specification [axis, index], where axis is
                   0=z, 1=y, 2=x and index is the slice position
                   (None selects the mid-plane)
    :type cutzyx: list [int, int or None]
    :param nameaxes: Names of the axes [z, y, x]
    :type nameaxes: list of str
    :param Ncolumns: Number of subplot columns
    :type Ncolumns: int
    :param title: Title prefix for each subplot
    :type title: str
    :param figblocksize: Size (in inches) of each subplot block
    :type figblocksize: float
    :param endread: Image file extension
    :type endread: str
    :param n0: Number of zero-padding digits in image indexing
    :type n0: int
    :param cmap: Colormap used for image display
    :type cmap: str
    :param interpolation: Matplotlib interpolation mode
    :type interpolation: str or None
    :param crop: Row cropping range applied to the cut image [start, stop]
    :type crop: list of int
...
def ProjVec3D(LCoord, Lv, cmap = 'viridis', colorbarlab=None, nameaxes=['z','y','x'], scale = 1, vmin = None, vmax = None):
     """
    Project and visualize 3D vector fields onto three orthogonal 2D planes.

    The vector field is displayed using quiver plots in the (x,y), (x,z),
    and (y,z) planes, with color indicating vector magnitude.

    :param LCoord: Coordinates of vectors (N, 3) as [z, y, x]
    :type LCoord: numpy.ndarray
    :param Lv: Vector components (N, 3) as [vz, vy, vx]
    :type Lv: numpy.ndarray
    :param cmap: Colormap used for vector magnitude
    :type cmap: str
    :param colorbarlab: Label for the colorbar
    :type colorbarlab: str or None
    :param nameaxes: Names of the axes [z, y, x]
    :type nameaxes: list of str
    :param scale: Scaling factor for quiver arrows
    :type scale: float
    :param vmin: Minimum magnitude for colormap normalization
    :type vmin: float or None
    :param vmax: Maximum magnitude for colormap normalization
